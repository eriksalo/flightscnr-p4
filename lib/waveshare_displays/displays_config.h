...
{0xE0, (uint8_t[]){0x00}, 1, 0},
    ...
    // esp_lcd_jd9365 sends these before the vendor table; Arduino_DSI_Display
    // replays only the raw table, so they must be included here. Without
    // COLMOD=0x55 the panel defaults to 24bpp and misinterprets the RGB565
    // stream (image vertically stretched ~1.5x, bottom cut off).
...
const DisplayConfig SCREEN_DEFAULT = {
    ...
    // Waveshare's BSP uses 80MHz (~108Hz refresh). That demands ~138MB/s of
    // sustained PSRAM reads for scanout; contention (CPU + C6 SDIO + draws)
    // caused DSI bridge underruns = dropped pixels / distorted geometry.
    // 48MHz ≈ 65Hz needs ~83MB/s and leaves headroom.
    .prefer_speed = 48000000,
    .lane_bit_rate = 1500,
    .width = 800,
    .height = 800,
    .rotation = 0,
