...
/** Hold the radar screen to reach Wi-Fi setup: an on-device scan list plus
 *  on-screen keyboard, so a new network needs nothing but the touchscreen.
 *  Saved networks are kept — the picker adds to them. */
void handleWifiSetupHold() {
  ...
  // Blocks while the setup screen is up, then either reboots onto the new
  // network or falls back here after cancel / the inactivity timeout.
  wifiOpenTouchSetupOnDemand();
...
void setup() {
  ...
  // and reboot the device. We intentionally run heavy work there, so watch only
  // core 1's idle task. (disableCore0WDT() is broken on IDF 5.x: it unsubscribes
  // the idle task but leaves the idle hook installed, which then floods the log
  // with "esp_task_wdt_reset: task not found" — reconfigure handles both.)
  {
    esp_task_wdt_config_t wdt_cfg = {};
    wdt_cfg.timeout_ms = 5000;
    wdt_cfg.idle_core_mask = 1 << 1;
    wdt_cfg.trigger_panic = true;
    esp_task_wdt_reconfigure(&wdt_cfg);
  }

  // Bring up the ESP-Hosted SDIO transport to the C6 BEFORE anything else
  // allocates: its mempool needs ~150KB of internal DMA RAM and asserts
  // (sdio_mempool_create) if services have already filled internal heap.
  // Everything after this spills to PSRAM as internal RAM runs low.
  Serial.println("WiFi transport warm-up (ESP-Hosted SDIO to C6)...");
  WiFi.mode(WIFI_STA);
  Serial.printf("[wifi] internal heap after hosted init: %u free\n",
                heap_caps_get_free_size(MALLOC_CAP_INTERNAL));
