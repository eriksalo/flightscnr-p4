...
/** Dev convenience: seed slot 0 from build flags when no networks are stored.
 *  Pass at build time (keeps credentials out of the repo):
 *    -D FLIGHTSCNR_WIFI_SSID='"MySSID"' -D FLIGHTSCNR_WIFI_PASS='"secret"'
 *  NVS persists the seed, so later builds without the flags keep working. */
void seedNetsFromBuildFlags() {
#if defined(FLIGHTSCNR_WIFI_SSID) && defined(FLIGHTSCNR_WIFI_PASS)
  ensureNetsLoaded();
  if (s_nets_count > 0) {
    return;
  }
  strncpy(s_nets[0].ssid, FLIGHTSCNR_WIFI_SSID, sizeof(s_nets[0].ssid) - 1);
  strncpy(s_nets[0].pass, FLIGHTSCNR_WIFI_PASS, sizeof(s_nets[0].pass) - 1);
  s_nets[0].fail_streak = 0;
  s_nets[0].demoted = false;
  s_nets_count = 1;
  persistNets();
  Serial.printf("[wifi] seeded network from build flags: %s\n", s_nets[0].ssid);
#endif
}
...
bool wifiSetupConnect() {
  ...
  Serial.printf("[wifi] internal heap: %u free, %u largest (hosted SDIO needs ~150KB)\n",
