...
/** Live DSI framebuffer as a 16bpp RGB565 BMP (debug: see the panel remotely). */
void handleScreenshot() {
  uint16_t* fb = displayFramebuffer();
  const int w = displayFbWidth();
  const int h = displayFbHeight();
  if (fb == nullptr || w <= 0 || h <= 0) {
    s_server->send(503, "text/plain", "framebuffer unavailable");
    return;
  }

  const uint32_t row_bytes = static_cast<uint32_t>(w) * 2;  // w*2 is 4-aligned
  const uint32_t data_bytes = row_bytes * static_cast<uint32_t>(h);
  const uint32_t header_bytes = 14 + 40 + 12;  // file + info + RGB565 masks
  uint8_t hdr[66] = {};
  ...
  hdr[0] = 'B';
  hdr[1] = 'M';
  put32(2, header_bytes + data_bytes);
  put32(10, header_bytes);
  put32(14, 40 + 12);                              // BITMAPINFOHEADER + masks
  put32(18, static_cast<uint32_t>(w));
  put32(22, static_cast<uint32_t>(-h));            // negative = top-down rows
  hdr[26] = 1;                                     // planes
  hdr[28] = 16;                                    // bpp
  ...
  s_server->setContentLength(header_bytes + data_bytes);
  s_server->send(200, "image/bmp", "");
  s_server->sendContent(reinterpret_cast<const char*>(hdr), sizeof(hdr));
  for (int row = 0; row < h; ++row) {
    s_server->sendContent(reinterpret_cast<const char*>(fb + static_cast<size_t>(row) * w),
                          row_bytes);
  }
}
