...
def main() -> None:
    ...
    total_verts = 0
    for kind in (KIND_MOTORWAY, KIND_PRIMARY, KIND_RIVER):
    ...
    counts = {k: sum(1 for kk, _ in out_paths if kk == k) for k in (0, 1, 2)}
...
// {counts[0]} motorway, {counts[1]} primary-road and {counts[2]} river paths.
...
enum : uint8_t {{ kPathMotorway = 0, kPathPrimary = 1, kPathRiver = 2 }};
